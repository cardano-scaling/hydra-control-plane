...
impl TxBuilder {
    ...
    pub fn cleanup_game(&self, utxos: Vec<UTxO>, network: Network) -> Result<BuiltTransaction> {
        ...
        for player in game_state.players {
            ...
            let inbound_address = player.inbound_address(network)?;
            let outbound_address = player.outbound_address(network)?;
            let inbound_script = player.inbound_script();
            let mut inbound_bytes = Vec::new();
            encode(&inbound_script, &mut inbound_bytes)?;
            let outbound_script = player.outbound_script();
